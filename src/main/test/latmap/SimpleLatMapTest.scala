...
class SimpleLatMapTest extends FunSuite with Matchers {
    test("SimpleLatMap basic functionality") {
        ...
        latmap.get(Array(1, 2, 3, 4, 5)) shouldBe DistLattice.Dst(7)
        latmap.get(Array(1, 2, 3, 4, 6)) shouldBe DistLattice.Infinity
        latmap.get(Array(6, 2, 3, 4, 5)) shouldBe DistLattice.NegInfinity
    }
}