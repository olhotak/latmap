...
import latmap.RuleElement.KeyVariable
import latmap.RuleElement.LatVariable
import latmap.RuleElement.Variable
  ...
  test("allocate variables") {
    ...
    val rule = Rule(
      new RuleElement {
        ...
        override def variables = Set(KeyVariable("a"), KeyVariable("c"), LatVariable("d"))
      ...
      List(
        ...
        new RuleElement {
          override def costEstimate(boundVars: Set[Variable]) = Int.MaxValue
          override def variables = Set(KeyVariable("a"), KeyVariable("b"), LatVariable("d1"))
          override def planElement(boundVars: Set[Variable], regAlloc: (Variable) => Int) = {
            IndexScan(index, mergeLat = false, Array(0, 1), Array(0, 1), 0)
          }
        },
        new RuleElement {
          override def costEstimate(boundVars: Set[Variable]) = Int.MaxValue
          override def variables = Set(KeyVariable("b"), KeyVariable("c"), LatVariable("d2"))
          override def planElement(boundVars: Set[Variable], regAlloc: (Variable) => Int) = {
            IndexScan(index, mergeLat = false, Array(0, 1), Array(0, 1), 0)
          }
        }
